  ...
  void initState() {
    ...
    availableCameras().then((availableCameras) {
      ...
      if (cameras.length > 0) {
        ...
        // I have only one camera on my device, so I set the selectedCameraIdx to 0. Don't do this on a real app!
        _initCameraController(cameras[0]).then((void v) {});
      } else {
        print("No camera available");
      }
    }).catchError((err) {
      print('Error catched: $err.code\nError Message: $err.message');
    });
  }
      ...
      builder: (context) => Material(
        child: CupertinoPageScaffold(
          navigationBar: CupertinoNavigationBar(
              leading: Container(), middle: Text('Edit taken images')),
          child: SafeArea(
            bottom: false,
        ...
        case Status.EYES_CLOSED:
          {
            ...
            print('[leftEyeOpenProbability] ' +
                face.leftEyeOpenProbability.toString());
            print('[rightEyeOpenProbability] ' +
                face.rightEyeOpenProbability.toString());
            break;
          }
        case Status.GLASSES:
          {
      ...
      if (error == null) {
        ...
        if (listOfStatus.isNotEmpty) {
          ...
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) =>
                  PreviewImageScreen(imagePaths: imagePaths.values.toList()),
            ),
          );
        }
      }
